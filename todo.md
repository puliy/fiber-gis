...
- [ ] Исправить рисование здания: начинать с первой точки, замыкать двойным кликом, сохранять
- [ ] Исправить сохранение кабеля (убедиться что CreateObjectDialog корректно вызывается)
- [ ] Улучшить UX рисования здания: показывать предварительный контур, кнопка отмены

### Фаза 2: Плавающий тулбар
- [ ] Сделать MapToolbar перетаскиваемым (drag-and-drop) по экрану
- [ ] Сохранять позицию тулбара в localStorage
- [ ] Увеличить размер кнопок тулбара

### Фаза 3: Новые инструменты тулбара
- [ ] Добавить инструмент add_mast (Мачта) в MapToolbar
- [ ] Добавить инструмент add_entry_point (Точка ввода) в MapToolbar
- [ ] Добавить инструмент add_flag (Флаг) в MapToolbar
- [ ] Добавить инструмент add_camera (Камера) в MapToolbar

### Фаза 4: Snap кабеля + вкладка Кабели
- [ ] При завершении рисования кабеля — автоматически находить ближайшую точку в радиусе 30м и сохранять startPointId/endPointId
- [ ] Добавить вкладку "Кабели" в ObjectDialog для точек (список кабелей через точку)
- [ ] Бэкенд: cables.byPoint — список кабелей, начинающихся или заканчивающихся в точке

### Фаза 5: Управление регионами + isPublic + роль editor
- [ ] AdminPage: вкладка "Регионы" — список, создание нового региона (форма)
- [ ] ObjectDialog: чекбокс isPublic в форме редактирования точки и кабеля
- [ ] AdminPage: добавить роль "editor" в список ролей при смене роли пользователя

### Фаза 6: Фильтр на карте + статистика
- [ ] Отдельная панель фильтров на карте (по статусу: Факт/План/Демонтаж, по типу объекта)
- [ ] Применять фильтры к отображаемым точкам и кабелям на карте
- [ ] AdminPage: статистика по объектам региона (кол-во точек по типам, кабели, длина)
